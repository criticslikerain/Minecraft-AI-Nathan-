...
Rename `keys.example.json` to `keys.json` and fill in your API keys, if none, make a file named keys.json. and you can set the desired model in `nathan.json` or other profiles.
...
You can configure the agent's name, model, and prompts in their profile like `nathan.json`.
...
Bot profiles are json files (such as `nathan.json`) that define:
